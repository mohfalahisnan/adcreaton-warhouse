...
import { addOutbound } from "@/lib/actions/outbound";
...
function OutboundForm() {
  ...
  const OutboundMutation = useMutation({
    mutationFn: async (data: Outbound & { product_name: string }) =>
      await addOutbound(data),
    onSuccess: () => {
    ...
    <div>
      <div className="flex justify-end">
        <ResponsiveDialog
          ...
          triggerContent={
            <div className="w-auto max-w-xs text-sm cursor-pointer flex bg-primary text-white items-center justify-center gap-2 px-4 py-2 rounded">
              Barang Keluar
            </div>
          }
        >
