...
import { getBarangMasuk } from "@/lib/actions/inbound";
...
function Pembayaran() {
  ...
  const thisday = useQuery({
    queryKey: ["barangmasuk"],
    queryFn: async () => await getBarangMasuk(parseFloat(warehouseId)),
  });
  ...
  return (
    <div className="border rounded-md p-4 bg-card">
      <h3 className="font-bold text-xl">
        Barang Masuk Hari ini : {thisday.data?.barang.length}
      </h3>
      <h3 className="font-bold text-xl">
        Barang Masuk Minggu ini : {thisday.data?.barangWeek.length}
      </h3>
